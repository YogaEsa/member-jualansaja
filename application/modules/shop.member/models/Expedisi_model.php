...
class Expedisi_model extends Base_Model{
  function __construct(){
      parent::__construct();
      $this->set_table('expedisi_pengiriman');
      $this->set_pk('id');
  }
  ...
  function getQB($Kondisi = "", $Limit = ""){
    $query= "select * from expedisi_pengiriman $Kondisi $Limit";
    return $this->db->query($query);
  }
  function queryComboBox($arrayKondisi = array()){
    $Kondisi = join(' and ', $arrayKondisi);
    $Kondisi = $Kondisi == '' ? '' : ' Where ' . $Kondisi;
    $query= "select * from expedisi_pengiriman $Kondisi";
    return $this->db->query($query);
  }
  function getLastID(){
      $this->db->select('id');
