...
class Manage extends MY_Controller {
	...
	public function index()
	{
		...
		$data['content'] = 'manage';
		$data['title'] = 'Leads';
		$arrayCustom = $this->Team_model->get_list("id_upline_direct = '".$this->session->userdata('id')."' ");
		$arrayData = array();
		$nomorUrut = 1;
	...
	public function filter(){
		...
		$data['comboStatus'] = $this->cmbArray("filterStatus",$levelMember,array(array("LEADS","LEADS"),array("RESELLER","RESELLER"),array("AGEN","AGEN")),"-- STATUS --","style='font-size: 14px;' class='form-control form-control-sm'");
	...
	public function daftar(){
		...
		$arrayCustom = $this->Team_model->getQB($this->generateCondition()," limit $numberStart,$jumlahData",$this->session->userdata('id'));
		foreach($arrayCustom->result_array() as $row){
			$getDataProvinsi = $this->Wilayah_model->get(array("provinsi_id_raja_ongkir"=>$row['id_provinsi'],"kota_id_raja_ongkir"=>"0","kecamatan_id"=>"0"));
			$getDataKota = $this->Wilayah_model->get(array("provinsi_id_raja_ongkir"=>$row['id_provinsi'],"kota_id_raja_ongkir"=>$row['id_kota'],"kecamatan_id"=>"0"));
			$arrayData[] = array(
				'id' => $row['id'],
				'nomor_daftar' => $nomorUrut,
				...
				'email' => $row['email'],
				'provinsi_kota' => $getDataKota['nama']." ".$getDataProvinsi['nama'],
				'username' => $row['affiliate'],
				'alamat' => $row['alamat'],
				'level_member' => $row['level_member'],
				'jumlah_member' => $row['jumlah_member'],
			);
			$nomorUrut += 1;
		}
		$data['arrayDaftar'] = $arrayData;
	...
	public function viewTeam(){
		...
		// 	$numberStart = $nomorUrut - 1;
		// }
		$arrayCustom = $this->Team_model->getQB("where id_upline_direct ='".$_GET['idMember']."'"," ",$_GET['idMember']);
		foreach($arrayCustom->result_array() as $row){
			$getDataProvinsi = $this->Wilayah_model->get(array("provinsi_id_raja_ongkir"=>$row['id_provinsi'],"kota_id_raja_ongkir"=>"0","kecamatan_id"=>"0"));
			$getDataKota = $this->Wilayah_model->get(array("provinsi_id_raja_ongkir"=>$row['id_provinsi'],"kota_id_raja_ongkir"=>$row['id_kota'],"kecamatan_id"=>"0"));
			$arrayData[] = array(
		...
		$data['arrayDaftar'] = $arrayData;
		$getDataMember = $this->Team_model->get($_GET['idMember']);
		$data['namaMember'] = $getDataMember['nama'];
		$this->load('view_team', $data['content'], $data);
	}
	function generateCondition(){
		...
		$arrayKondisi[] = "id_upline_direct = '".$this->session->userdata('id')."'";
	...
	public function paging(){
		$getData = $this->Team_model->getQB($this->generateCondition(),"",$this->session->userdata('id'));
		$pageKe = $_POST['pageKe'];
	...
	function save() {
		...
		$this->Team_model->save($dataInput);
